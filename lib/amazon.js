...
const SEARCHES = [
  ...
  ,{ keywords: 'Katzen Trinkbrunnen Edelstahl', pet: 'cat', category: 'Trinkbrunnen' }
  ,{ keywords: 'Katzen Futterautomat Chip', pet: 'cat', category: 'Futterautomaten' }
  ,{ keywords: 'Katzenklo automatisch App', pet: 'cat', category: 'Katzenklo-Roboter' }
  ,{ keywords: 'Katzenspielzeug automatisch interaktiv', pet: 'cat', category: 'Automatische Spielzeuge' }
  ,{ keywords: 'Katzen Kamera mit App', pet: 'cat', category: 'Tierkameras' }
  ,{ keywords: 'Katzen GPS Halsband Tracker', pet: 'cat', category: 'GPS-Tracker' }
  ,{ keywords: 'Futterautomat Katze Kamera', pet: 'cat', category: 'Futterautomaten' }
  ,{ keywords: 'Katzenklappe Mikrochip App', pet: 'cat', category: 'Mikrochip-Katzenklappen' }
  ,{ keywords: 'Hund GPS Tracker Halsband', pet: 'dog', category: 'GPS-Tracker' }
  ,{ keywords: 'Hundekamera Leckerli App', pet: 'dog', category: 'Tierkameras' }
  ,{ keywords: 'Futterautomat Hund Kamera', pet: 'dog', category: 'Futterautomaten' }
  ,{ keywords: 'Hunde Trinkbrunnen Edelstahl', pet: 'dog', category: 'Trinkbrunnen' }
  ,{ keywords: 'Hundebürste elektrisch Tierhaar', pet: 'dog', category: 'Pflegegeräte' }
  ,{ keywords: 'Hundeschermaschine Profi leise', pet: 'dog', category: 'Pflegegeräte' }
  ,{ keywords: 'Hundetrainer Ultraschall Gerät', pet: 'dog', category: 'Trainingsgeräte' }
  ,{ keywords: 'Hundeklappe elektronisch Chip', pet: 'dog', category: 'Smarte Hundeklappen' }
  ,{ keywords: 'Haustier Luftreiniger Tierhaare', pet: 'both', category: 'Raumluft & Tierhaare' }
  ,{ keywords: 'Tierhaar Staubsauger Haustier', pet: 'both', category: 'Tierhaar-Saugsysteme' }
  ,{ keywords: 'Haustier Kamera 360 Grad', pet: 'both', category: 'Tierkameras' }
  ,{ keywords: 'Futterautomat Haustier App', pet: 'both', category: 'Futterautomaten' }
];

const REQUEST_PLAN = SEARCHES.flatMap(search =>
  Array.from({ length: SEARCH_PAGES }, (_, index) => ({ search, page: index + 1 }))
);
const TOTAL_BATCHES = Math.ceil(REQUEST_PLAN.length / REQUESTS_PER_BATCH);
...
async function loadProductByAsin(asin, fetchImpl = fetch) {
  ...
  const data = await response.json();
  const item = (data.itemsResult?.items || []).find(entry => entry.asin === normalized);
  const product = item ? mapItem(item, { pet: 'both', category: 'Haustiertechnik' }) : null;
...
async function loadProducts(fetchImpl = fetch, batch = 0) {
  const safeBatch = Math.max(0, Math.min(TOTAL_BATCHES - 1, Number.parseInt(batch, 10) || 0));
  const productCache = productCaches.get(safeBatch);
  if (productCache && productCache.expiresAt > Date.now()) {
    return { ...productCache.payload, cached: true };
  }
  const accessToken = await getAccessToken(fetchImpl);
  const requests = REQUEST_PLAN.slice(safeBatch * REQUESTS_PER_BATCH, (safeBatch + 1) * REQUESTS_PER_BATCH);
  const batches = [];
  for (let index = 0; index < requests.length; index += 1) {
    batches.push(await searchProducts(requests[index].search, accessToken, fetchImpl, requests[index].page));
    if (index < requests.length - 1) await delay(SEARCH_DELAY_MS);
  }
  const products = deduplicateVariants(batches.flat()).slice(0, MAX_PRODUCTS);
  const payload = { products, count: products.length, batch: safeBatch, totalBatches: TOTAL_BATCHES, fetchedAt: new Date().toISOString() };
  productCaches.set(safeBatch, { payload, expiresAt: Date.now() + PRODUCT_CACHE_MS });
